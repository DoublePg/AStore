...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  typedef typename Alloc::template rebind<self_type>::other alloc_type;//用于分配数据节点
  typedef typename Alloc::template rebind<T>::other key_alloc_type;//用于分配键
  typedef typename Alloc::template rebind<P>::other payload_alloc_type;//用于分配负载
  typedef typename Alloc::template rebind<V>::other value_alloc_type;//用于分配k-v对
  typedef typename Alloc::template rebind<uint64_t>::other bitmap_alloc_type;//用于分配位图
...
int buf_ptr = 0;
  ...
  void update_payload(int pos, const P &payload){
...
#if ALEX_DATA_NODE_SEP_ARRAYS
    key_slots_ =
        //new (Allocate.alloc(data_capacity_ * sizeof(T))) T[data_capacity_];
        new (key_allocator().allocate(data_capacity_)) T[data_capacity_];
    payload_slots_ =
        new (payload_allocator().allocate(data_capacity_)) P[data_capacity_];
        //new (Allocate.alloc(data_capacity_ * sizeof(P))) P[data_capacity_];
#else
    data_slots_ =
        new (value_allocator().allocate(data_capacity_)) V[data_capacity];
#endif
    key_buf = new (key_allocator().allocate(buf_size)) T[buf_size];
    payload_buf = new (payload_allocator().allocate(buf_size)) P[buf_size];
  ...
  void bulk_load(const V values[], int num_keys,
    ...
    for (int i = 0; i < num_keys; i++) {
      ...
      int predict_pos = position;
      position = std::max<int>(position, last_position + 1);
      if(position - predict_pos > 0)
      {
	      if(position - predict_pos > max_error)
		      max_error = position - predict_pos;
      }
       if(position - predict_pos < 0)
      {
	      if(position - predict_pos < min_error)
		      min_error = position - predict_pos;
      }
       /*if(values[i].first == 1889249340051071488)                 
       {
	       RDMA_LOG(4)<<"pos is "<<position;
	       RDMA_LOG(4)<<"predict pos is "<<predict_pos;
       }*/
...
#endif
          set_bit(pos);
	  predict_pos = this->model_.predict(values[j].first);
	  if(values[j].first == 1889249340051071488)
	  {
		  RDMA_LOG(4)<<"pos is "<<pos;
		  RDMA_LOG(4)<<"predict pos is "<<predict_pos;
	  }
	  if(pos - predict_pos > 0)
	  {
		  if(pos - predict_pos > max_error)
			  max_error = pos - predict_pos;
	  }
	  if(pos - predict_pos < 0)
	  {
		  if(pos - predict_pos < min_error)
			  min_error = pos - predict_pos;
	  }
          pos++;
        }
        last_position = pos - 1;
  ...
  void bulk_load_from_existing(
    ...
    for (; it.cur_idx_ < right && !it.is_end(); it++) {
      ...
      int predict_pos = position;
      position = std::max<int>(position, last_position + 1);
      predict_pos = std::min<int>(std::max<int>(predict_pos, 0),data_capacity_ - 1);
      if(position - predict_pos > 0)
      {
	      if(position - predict_pos > 0)
	      {
...
#endif
          set_bit(pos);
	  predict_pos = this->model_.predict(it.key());
	  predict_pos = std::min<int>(std::max<int>(predict_pos, 0),data_capacity_ - 1);
	  if(pos - predict_pos > 0)
	  {
		  if(pos - predict_pos > max_error)
  ...
  int find_key(const T& key) {
   ...
   // LOG(2)<<"predict pos is "<<predicted_pos;
  ...
  int lower_bound(const K& key) {
    num_lookups_++;
    int position = predict_position(key);
  //  LOG(2)<<"model a is "<<this->model_.a_;
   // LOG(2)<<"model b is "<<this->model_.b_;
  ...
  std::pair<int, int> insert(const T& key, const P& payload) {
    ...
    if (insertion_position < data_capacity_ &&
      ...
      insert_element_at(key, payload, insertion_position,is_expand);

      int predict_pos = predict_position(key);
      if (insertion_position - predict_pos > max_error) //成功更新需要更新最大最小误差
      {
	     this-> max_error = insertion_position - predict_pos;
      }
      if (insertion_position - predict_pos < min_error)
      {
	      this->min_error = insertion_position - predict_pos;
      }
    } else {
      //LOG(2)<<"insert using s";
      insertion_position =
          insert_using_shifts(key, payload, insertion_position,is_expand);
    }
  ...
  void resize(double target_density, bool force_retrain = false,
    ...
    for (; it.cur_idx_ < data_capacity_ && !it.is_end(); it++) {
	  ...
	  predict_pos = this->model_.predict(it.key());
	  predict_pos =  std::min<int>(std::max<int>(predict_pos, 0),new_data_capacity - 1); 
	  //LOG(2)<<"pos is "<<pos<<" "<<"predict_pos is "<<predict_pos;
    if(pos - predict_pos > 0)
	  {
		  if(pos - predict_pos > max_error)
			  max_error = pos - predict_pos;
    ...
    expansion_threshold_ =
        std::min(std::max(data_capacity_ * kMaxDensity_,
                 ...
                 static_cast<double>(data_capacity_));
    contraction_threshold_ = data_capacity_ * kMinDensity_;
    LOG(2)<<"max err is "<<max_error<<" "<<"min err is "<<min_error;
  }
  ...
  void insert_element_at(const T& key, P payload, int pos, bool is_expand) {//修改RDMA区域在此，可能有bug
  if(!is_expand)
{
  LOG(2)<<"insert key is "<<key;
  #if ALEX_DATA_NODE_SEP_ARRAYS
    key_slots_[pos] = key;
...
#endif
    set_bit(pos);
    char* temp_addr = (char*)this->RDMA_Addr + 2 * sizeof(int);
    char* temp_addr1 = temp_addr + sizeof(T) * pos;
    memcpy(temp_addr1, &key,sizeof(T));
    memcpy(temp_addr + sizeof(T) * this->data_capacity_ + sizeof(P) * pos,&payload,sizeof(P));
  ...
  int insert_using_shifts(const T& key, P payload, int pos, bool is_expand) {
    // Find the closest gap
    
    if(is_expand) //扩展过，统一处理RDMA区域
    {
	...
	{
		int predict_pos = predict_position(key_slots_[i]);
    predict_pos = std::min<int>(std::max<int>(predict_pos,0),data_capacity_-1);   
    if(i - predict_pos > 0)
    {
      if(i - predict_pos > max_error)
      {
          max_error = i - predict_pos;
      }
    }
	      
      if(i - predict_pos < 0)
      {
        if(i - predict_pos < min_error)
        {
          min_error = i - predict_pos;
        }
      }
	}
#else
      ...
      insert_element_at(key, payload, pos,is_expand);
	...
	else
	{
		int predict_pos = predict_position(key_slots_[i]);
		if(i - predict_pos > max_error)
		{max_error = i - predict_pos;LOG(2)<<this->no;}
		if(i - predict_pos < min_error)
			min_error = i - predict_pos;           
	}
#else
      ...
      //LOG(2)<<this->no;
      insert_element_at(key, payload, pos - 1,is_expand);
      num_shifts_ += pos - gap_pos - 1;
     int predict_pos = predict_position(key);     
      if(pos- 1 - predict_pos > max_error)
      { max_error = pos - 1 - predict_pos;}
      if(pos - predict_pos < min_error)
	      min_error = pos- 1 - predict_pos;
      return pos - 1;
    }
  }
  else //未扩展，单独处理RDMA区域
  {
    ...
    set_bit(gap_pos);
    char* temp_addr =(char*) this->RDMA_Addr + 2 * sizeof(int);
    temp_addr += sizeof(T) * gap_pos;
    char* temp_addr1 =(char*) this->RDMA_Addr + 2 * sizeof(int) + this->data_capacity_*sizeof(T);
    temp_addr1 += sizeof(P) * gap_pos;
    if (gap_pos >= pos) {
      for (int i = gap_pos; i > pos; i--) {
#if ALEX_DATA_NODE_SEP_ARRAYS
        key_slots_[i] = key_slots_[i - 1];
        memcpy(temp_addr,&key_slots_[i],sizeof(T));
        temp_addr -= sizeof(T);
        payload_slots_[i] = payload_slots_[i - 1];
        memcpy(temp_addr1,&payload_slots_[i],sizeof(P));
        temp_addr1 -= sizeof(P);
	if(!check_exists(i))
		          {      continue;}      
	else
