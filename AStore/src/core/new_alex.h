...
#include <cmath>
#include "alex_base.h"
#include "new_fanout_tree.h"
#include "new_alex_node.h"
...
namespace alex
{
  ...
  public:
    ...
    Alex(const Alloc &alloc) : allocator_(alloc)
    {
     ...
     // auto empty_data_node = new (data_node_allocator().allocate(1))
       //   data_node_type(key_less_, allocator_);
      auto empty_data_node = new (reinterpret_cast<data_node_type*>(alloc.alloc_one(sizeof(data_node_type))))  
                    data_node_type(key_less_, allocator_);
      empty_data_node->bulk_load(nullptr, 0);
      root_node_ = empty_data_node;
      create_superroot();
      ...
      while (1)
      {
        ...
        else
        {
          ...
          if (cur->no == 0)
          {
            ...
            LOG(2) << "the no is " << cur->no << " "
                   << "the addr is " << tp_addr.addr;
...
// node's parent.
  ...
  public:
   ...
   /*template<class Alloc1>
    void bulk_load(const V values[], int num_keys,Alloc1& alloc)
    {
      ...
      delete_node(root_node_); // delete the empty root node from constructor
       typedef AlexModelNode<T, P, Alloc1> model_node_type1;
      stats_.num_keys = num_keys;
      root_node_ = new (reinterpret_cast<model_node_type1*>(alloc.alloc_one(sizeof(model_node_type)))) model_node_type(0,alloc);  
  ...
  private:
        ...
        bool reuse_model)
    {
      ...
      //LOG(2)<<"opppppppppppppppppppppppppppp";
      delete_node(leaf);
      stats_.num_data_nodes--;
      stats_.num_model_nodes++;
        ...
        bool reuse_model)
    {
      ...
      if (used_fanout_tree_nodes.empty())
      {
        ...
        create_two_new_data_nodes(
            leaf, parent, std::max(fanout_tree_depth, static_cast<int>(leaf->duplication_factor_)),
                                   ...
                                   int start_bucketID = 0)
    {
      ...
      int appending_left_bucketID = std::min<int>(
          ...
          parent->num_children_ - 1);
      /*LOG(2)<<old_node->no;
LOG(2)<<"parent model a is "<<parent->model_.a_;
LOG(2)<<"parent model b is "<<parent->model_.b_;
LOG(2)<<"mid_bucketID is "<<mid_bucketID;*/
      int right_boundary = old_node->lower_bound(
          (mid_bucketID - parent->model_.b_) / parent->model_.a_);
      // LOG(2)<<"assssssssssssssss";
