...
class Alex {
 ...
 public:
  ...
  std::vector<address> ad; 
  char *model_buf_start;
  char *model_buf_cur;
  char *data_buf_cur;
  char *data_buf_start;
  char *struct_buf_start;	
  int model_region_size = 0;
  int white_size = 0;  
  int buf_size = 150;
  bool expand_flag = false;
  int err_info[100];
  struct insert_after_split{
	  data_node_type* leaf;
//	  int leaf_pos;
  };
  std::vector<u64> model_address;
  std::vector<insert_after_split> split_node;
  //  address ad[20000];
  ...
  std::vector<AlexNode<T,P>*> node_index;
 ...
 public:
  ...
  void collect_statics_info(AlexNode<T,P>* node)
  {
    std::queue<AlexNode<T,P>*> myqueue;
    ...
    while (1)
	  {
		    if (!node->is_leaf_)//非叶节点直接跳过
		    {
          auto cur = static_cast<model_node_type*>(node); 
          for (int i = 0; i < cur->num_children_; i++)
		    ...
		    {
			      auto cur = static_cast<data_node_type *>(node);
            //LOG(2)<<"data node";
			      for(int i = 0; i < cur->data_capacity_; i++)
            {
              if(cur->check_exists(i) == true)
              {
                 auto key = cur->get_key(i);
                 int predict_pos = cur->predict_position(key);
                 int err = abs(i - predict_pos);
                 if(err == 0)
                 {
                    err_info[0]++;
                 }
    ...
    myqueue.pop();
		...
		for(int i = 0; i < 100; i++)
    {
      LOG(2)<<"i is "<<i<<" "<<"err is "<<err_info[i];
    }
	}
  int get_buf_size()
  {
  ...
  void set_no( AlexNode<T,P>* node)
  {
    std::queue<AlexNode<T,P>*> myqueue;
    int k = 1;
    int repeats;
    node_index.push_back(node);
    if(node){  node->no = 1; node -> version = 1; node_index.push_back(node); }				          
    if(!node->is_leaf_)
    {
       k++;
       myqueue.push(node);
       ...
       {       
         ...
         for(int i = 0; i < cur->num_children_; i++)
          {    	  
            if(cur->children_[i]->is_leaf_)
            {
              cur->children_[i]->no = k;
	      cur->children_[i]->version = 1;
              repeats = 1 << cur->children_[i]->duplication_factor_;
	      node_index.push_back(cur->children_[i]);
	      i+=repeats-1;
	      k++;
              continue;
            }
   ...
   RDMA_LOG(4)<<"The capa od is "<<node_index.size();
}

    
void set_no_with_address(AlexNode<T, P> *node)
{
	std::queue<AlexNode<T, P> *> myqueue;
	int k = 1;
	int j = 0;
	...
	while (1)
	{
		...
		else
		{
			...
			if (cur->no == 0)
			{
				char *temp_addr = data_buf_cur;
		 		cur->no = k;
			  k++;
				//LOG(2)<<"split no is "<<cur->no;
				address tp_addr;
				tp_addr.addr = (u64)temp_addr;
        cur->RDMA_Addr = tp_addr.addr;//为了在叶子中实现插入RDMA区域，故每个叶子保存其RDMA区域对应地址
				LOG(2)<<"the no is "<<cur->no<<" "<<"the addr is "<<tp_addr.addr;
				tp_addr.data_capacity = cur->data_capacity_;
				if(cur->no == 1368)
					LOG(2)<<"the data capa "<<cur->data_capacity_;
				tp_addr.no = cur->no;
				//memcpy(temp_addr, &cur->data_capacity_, sizeof(int));
				memcpy(temp_addr, &cur->no, sizeof(int));
				temp_addr += sizeof(int);
				memcpy(temp_addr, &cur->version, sizeof(int));
				temp_addr += sizeof(int);
				//int buf_element = -1;
				for (int i = 0; i < cur->data_capacity_; i++)
				{
					T tmp1 = cur->key_slots_[i];
					memcpy(temp_addr, &cur->key_slots_[i], sizeof(T));
					temp_addr += sizeof(T);									
				}
				for (int i = 0; i < cur->data_capacity_; i++)
				{
					T tmp2 = cur->payload_slots_[i];
					memcpy(temp_addr, &tmp2, sizeof(P));
					temp_addr += sizeof(P);
				}
				tp_addr.sz = temp_addr - data_buf_cur;
		  		//LOG(2)<<"tp_addr size is "<<tp_addr.sz;
				ad.insert(ad.begin()+j,tp_addr);
				cur->struct_no = j;
				j++;
				
				data_buf_cur = temp_addr;
				ASSERT((u64)data_buf_cur < (u64)model_buf_start);
			       	insert_after_split tmp;
				tmp.leaf = cur;
				//tmp.leaf_pos = j - 1;
				split_node.push_back(tmp); 
			}
			else //将结构中原存在的数据节点的编号改为新编号
			{
				if (ad[j].no == cur->no)
				{
					cur->no = k;
					ad[j].no = k;
                                        //if(ad[j].no == 1370)
		                        //LOG(2)<<"1370 "<<ad[j].addr;
					char* tp_ad = (char*)ad[j].addr;//当编号修改时，修改对应的RDMA区域的编号
					memcpy(tp_ad,&cur->no,sizeof(int));
					cur->struct_no = j;
					j++;
					k++;
				...
				else
				{
					...
					if (ad[j].no < cur->no)
					{
						...
						cur->no = k;
						ad[l].no = k;
                                                RDMA_LOG(4)<<ad[j].no<<"j is "<<j;
                                          						
						k++;
					}
					if (ad[j].no > cur->no)
					{
						RDMA_LOG(4)<<"ad[j].no is "<<ad[j].no;
					        RDMA_LOG(4)<<"ad[j-1].no is "<<ad[j-1].no;
						RDMA_LOG(4)<<"j is "<<j;
						RDMA_LOG(4)<<"cur->no is "<<cur->no;
						ASSERT(false);
						for (l = j; l < ad.size(); l++)
	...
	auto temp_addr = struct_buf_start;
	for (int i = 0; i < ad.size(); i++)
	{
		memcpy(temp_addr, &ad[i], sizeof(address));
		temp_addr += sizeof(address);
	}
	//RDMA_LOG(4)<<"ad.size() is "<<ad.size();
	//RDMA_LOG(4)<<"the num of data_node is "<<stats_.num_data_nodes;
	//RDMA_LOG(4)<<"ad[0].no is "<<ad[ad.size()-1].no;
	model_region_size = temp_addr - model_buf_start;
}
...
bool update_payload(const T &key, const P &payload) 
{
	data_node_type *leaf = get_leaf(key);
	int idx = leaf->find_key(key);
	if(idx < 0)
	{
		for(int i = leaf->buf_ptr - 1; i >= 0; i--)
		{
			if(leaf->key_buf[i] == key)
			{
                            leaf->payload_buf[i] = payload;
			    return true;
			}
		}
		return false;
	...
	{
		leaf->update_payload(idx,payload);
		char* addr = (char*)ad[leaf->struct_no].addr;
		addr += 2 * sizeof(int) + leaf->data_capacity_ * sizeof(T) + idx * sizeof(P);
		memcpy(addr,&payload,sizeof(P));
		return true;
	}
}


bool update_payload_with_no(const T &key,const P &payload, const int no)
{
        data_node_type *leaf = static_cast<data_node_type*>(node_index[no]);
//	data_node_type *leaf = get_leaf(key);
	int idx = leaf->find_key(key);
	if(idx < 0)
	{
...
void reset_data_region()	    
{	
      	auto node = root_node_;
	char *temp_addr = data_buf_start;
	int repeats;
	ad.clear();
	RDMA_LOG(4)<<"reset data region";
	RDMA_LOG(4)<<(u64)temp_addr;
	std::queue<alex::AlexNode<T, P> *> myqueue;
	myqueue.push(node);
	while (1)
	{
		if (!node->is_leaf_)
		...
		{
		        auto cur1 = static_cast<data_node_type *>(node);
			address temp_ad;
			temp_ad.addr = (u64)temp_addr;
			temp_ad.data_capacity = cur1->data_capacity_;
			temp_ad.no = cur1->no;
			//memcpy(temp_addr, &cur1->data_capacity_, sizeof(int));
			memcpy(temp_addr,&cur1->no,sizeof(int));
			temp_addr += sizeof(int);
			memcpy(temp_addr, &cur1->version, sizeof(int));
			temp_addr += sizeof(int);
			//T buf_element = -1;
			for (int i = 0; i < cur1->data_capacity_; i++)
			{
				T tmp1 = cur1->key_slots_[i];
				memcpy(temp_addr, &tmp1, sizeof(T));
				temp_addr += sizeof(T);
			}
	/*		for (int i = 0; i < buf_size; i++)
			{
	                	memcpy(temp_addr, &buf_element, sizeof(T));
				temp_addr += sizeof(T);
                         }
		P buf_element1 = -1;*/
             for (int i = 0; i < cur1->data_capacity_; i++)
	     {
		     P tmp2 = cur1->payload_slots_[i];
		     memcpy(temp_addr, &tmp2, sizeof(P));
		     temp_addr += sizeof(P);
	     }
	    /* for (int i = 0; i < buf_size; i++)
	     {
		     memcpy(temp_addr, &buf_element1, sizeof(P));
		     temp_addr += sizeof(P);
	     }
	     for (int i = 0; i < cur1->bitmap_size_; i++)
	     {
		     uint64_t tmp3 = cur1->bitmap_[i];
		     memcpy(temp_addr, &tmp3, sizeof(uint64_t));
		     temp_addr += sizeof(uint64_t);
	     }*/		
	     temp_ad.sz = (u64)temp_addr - temp_ad.addr;
	     cur1->struct_no = ad.size() - 1;
	...
	data_buf_cur = temp_addr;
	RDMA_LOG(4)<<"The size of data is "<<data_buf_cur - data_buf_start;
	temp_addr = struct_buf_start;
	for(int i = 0; i < ad.size(); i++)
	{
		memcpy(temp_addr,&ad[i],sizeof(address));
		temp_addr += sizeof(address);
...
void change_RDMA_Region(int leaf_pos, data_node_type *leaf)
{
	char *temp_addr = reinterpret_cast<char *>(ad[leaf_pos].addr);
  ASSERT(leaf->RDMA_Addr == (u64)temp_addr);
	temp_addr += sizeof(int);
  memcpy(temp_addr, &leaf->version, sizeof(int)); //修改对应RDMA区域中的版本号
    	//T buf_element = -1;
  temp_addr += sizeof(int);
	for (int i = 0; i < leaf->data_capacity_; i++)
  {
		memcpy(temp_addr, &leaf->key_slots_[i], sizeof(T));
		temp_addr += sizeof(T);
	}
  	for (int i = 0; i < leaf->data_capacity_; i++)
	{
		memcpy(temp_addr, &leaf->payload_slots_[i], sizeof(P));
		temp_addr += sizeof(P);
	}
}
//bug
bool insert_different_version(T key, P value)
{
      h:bool resize_flag = false;
	bool is_split = false;
	//split_node.clear();
	//std::vector<data_node_type *> resize_node;
	std::vector<TraversalNode> traversal_path;
	 data_node_type *leaf = get_leaf(key);
	...
	if(leaf->no == 724)
	{	LOG(2)<<leaf->model_.a_;
	        LOG(2)<<leaf->model_.b_;
	}
	if (leaf->buf_ptr < buf_size)
	{
		leaf->key_buf[leaf->buf_ptr] = key;
		leaf->payload_buf[leaf->buf_ptr] = value;
		leaf->buf_ptr++;
		return true;
	}
	else
	{
		...
		char* ptr = reinterpret_cast<char*>(ad[leaf->struct_no].addr);
		int capa;
		capa = leaf->data_capacity_;
		LOG(2)<<"uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu";
		ptr += sizeof(int) * 2;
      		RDMA_Key_Addr = ptr;
		ptr += sizeof(int) * capa;		
       		RDMA_Payload_Addr = ptr;
		//LOG(2)<<"leaf->no is "<<leaf->no;
       //         for(int i = 0 ; i < buf_size; i++)
	//		LOG(2)<<leaf->key_buf[i];
		insert_buf(leaf->no,parent->no,RDMA_Key_Addr,RDMA_Payload_Addr,reinterpret_cast<char*>(model_address[leaf->no]) ,leaf->struct_no);
		//LOG(2)<<"leaf->no is "<<leaf->no;
		goto h;
		//insert_buf(insert_element.no, insert_element.parent_no, RDMA_Key_Addr, RDMA_Payload_Addr,reinterpret_cast<char*>(index1.model_address[insert_element.no]),insert_element.struct_no);
	}
	/*std::pair<int, int> ret = leaf->insert(key, value);
	int fail = ret.first;
	...
	int capa;
	capa = leaf->data_capacity_;
	//memcpy(&capa,ptr,sizeof(int));
	ptr += sizeof(int) * 2;
	RDMA_Key_Addr = ptr;
	//ptr += sizeof(int) * (buf_size + capa);
        ptr += sizeof(int) * capa;
	RDMA_Payload_Addr = ptr;
	if (fail == -1)
	{
	...
	int tmp = leaf->num_resizes_;
	if (fail == 0)
	{
		if (leaf->num_resizes_ > tmp)
		{
			leaf->version++;
			char *temp_addr = reinterpret_cast<char *>(model_address[leaf->no]);
			resize_RDMA_Region(leaf->struct_no,leaf, temp_addr, RDMA_Key_Addr, RDMA_Payload_Addr, is_split);
			memset(leaf->key_buf, -1, sizeof(leaf->key_buf));
			memset(leaf->payload_buf, -1, sizeof(leaf->payload_buf));
			LOG(2)<<"resize "<<leaf->no;
			leaf->buf_ptr = 0;
			return true;
		}
		memset(leaf->key_buf, -1, sizeof(leaf->key_buf));
		memset(leaf->payload_buf, -1, sizeof(leaf->payload_buf));
		leaf->buf_ptr = 0;
		change_RDMA_Region(leaf->struct_no, leaf);
		return true;
	}
	if (fail)
	{
		...
		while (fail)
		{
			...
			if (fanout_tree_depth == 0)
			{
				...
				resize_flag = true;
				LOG(2)<<leaf->no<<" resize";
				if (is_split == true)
				{
					resize_node.push_back(leaf);
				}
			}
			else
			{
				...
				else
				{
					...
					if (is_split)
					{
						...
						{
							if (split_node[i].leaf->no == leaf->no)
							{
								split_node.erase(split_node.begin() + i);
							}
						}
					}
					int struct_no = leaf->struct_no;
					split_sideways(parent, bucketID, fanout_tree_depth,
							used_fanout_tree_nodes, reuse_model);
					is_split = true;
					for (int i = 0; i < resize_node.size(); i++)
					{
						if (resize_node[i]->no == leaf->no)
						{
							resize_node.erase(resize_node.begin() + i);
						}
					}
					char *temp_addr = reinterpret_cast<char *>(ad[leaf->struct_no].addr); //当客户端进行RDMA读时，就需要重新拉模型。
					temp_addr += sizeof(int);                                       //更改RDMA区域内的版本号
					int temp_version = 999;
					memcpy(temp_addr, &temp_version, sizeof(int));
					ad.erase(ad.begin() + struct_no); //删除结构中的分裂前的>元素
					temp_addr = model_buf_start;      //重置RDMA模型区域
					auto node = root_node_;
					int tmp_size;
					tmp_size = split_node.size();
					set_no_with_address(root_node_);
				}
	...
	{
		auto node = root_node_;
		char *temp_addr = model_buf_start;
		std::queue<AlexNode<T, P> *> myqueue;
		myqueue.push(node);
		int repeats = 0;
		int nu_node = 0;
		int r = 0;
		model_address.clear();
		model_address.push_back((u64)temp_addr);
		while (1)
		{
			if (!node->is_leaf_)
			{
				...
				memcpy(temp_addr, &cur->no, sizeof(int));
				temp_addr += sizeof(int);
				memcpy(temp_addr, &cur->model_.a_, sizeof(double));
				temp_addr += sizeof(double);
				memcpy(temp_addr, &cur->model_.b_, sizeof(double));
				temp_addr += sizeof(double);
				memcpy(temp_addr, &cur->num_children_, sizeof(int));
				temp_addr += sizeof(int);
				char *temp_addr1;
				temp_addr1 = temp_addr;
				temp_addr += sizeof(int);
				for (int i = 0; i < cur->num_children_; i++)
				{
					myqueue.push(cur->children_[i]);
					repeats = 1 << cur->children_[i]->duplication_factor_;
					memcpy(temp_addr, &cur->children_[i]->no, sizeof(int));
					temp_addr += sizeof(int);
					memcpy(temp_addr, &repeats, sizeof(int));
					temp_addr += sizeof(int);
					i += repeats - 1;
				}
				int length = (temp_addr - temp_addr1 - sizeof(int)) / sizeof(int);
				memcpy(temp_addr1, &length, sizeof(int));
			}
			else
			{
				auto cur = static_cast<data_node_type *>(node);
				...
				temp_addr += sizeof(double);
				int y = 0;
				memcpy(temp_addr, &y, sizeof(int));
				temp_addr += sizeof(int); //删除了版本号，
				memcpy(temp_addr, &cur->version, sizeof(int));
				temp_addr += sizeof(int);
				memcpy(temp_addr, &cur->min_error,sizeof(int));
				temp_addr += sizeof(int);
				memcpy(temp_addr, &cur->max_error,sizeof(int));
				temp_addr += sizeof(int);
			}
			myqueue.pop();
			if (myqueue.empty())
		...
		white_size = struct_buf_start - temp_addr;
		for (int i = 0; i < resize_node.size(); i++)
		{
			auto cur = resize_node[i];
			for (int j = 0; i < split_node.size(); j++)
			{
				if (split_node.at(j).leaf->no == resize_node[i]->no)
				{
					split_node.erase(split_node.begin() + j);
					break;
				}
			}
			char *temp_addr = reinterpret_cast<char *>(model_address[resize_node[i]->no]);
			resize_RDMA_Region(resize_node[i]->struct_no, cur, temp_addr, RDMA_Key_Addr, RDMA_Payload_Addr, is_split);
		}
		for (int i = 0; i < split_node.size(); i++)
		{
			change_RDMA_Region(split_node.at(i).leaf->struct_no, split_node.at(i).leaf);
		}
	}
	 if(model_buf_start - data_buf_cur < 10000)	       
	 {
		 reset_data_region();
	 }
	return true;*/
}
void resize_RDMA_Region(int leaf_pos, data_node_type *leaf,char* leaf_model_buf_start,char*&RDMA_Key_Addr,char*&RDMA_Payload_Addr,bool is_split)
{
	char *temp_addr = reinterpret_cast<char *>(ad[leaf_pos].addr);
   ...
   temp_addr += sizeof(int);
	...
	memcpy(temp_addr, &leaf->version, sizeof(int)); //修改对应RDMA区域中的版本号	
	temp_addr = data_buf_cur;
  leaf->RDMA_Addr = (u64)temp_addr;
//	memcpy(temp_addr, &leaf->data_capacity_, sizeof(int));
  memcpy(temp_addr, &leaf->no,sizeof(int));
	temp_addr += sizeof(int);
	memcpy(temp_addr, &leaf->version, sizeof(int));
	//T buf_element = -1;
	temp_addr+=sizeof(int);
	RDMA_Key_Addr = temp_addr;
	for (int i = 0; i < leaf->data_capacity_; i++)
	{
		T tmp1 = leaf->key_slots_[i];
		memcpy(temp_addr, &tmp1, sizeof(T));
		temp_addr += sizeof(T);
	}
	RDMA_Payload_Addr = temp_addr;
	for (int i = 0; i < leaf->data_capacity_; i++)
	{
		P tmp2 = leaf->payload_slots_[i];
		memcpy(temp_addr, &tmp2, sizeof(P));
		temp_addr += sizeof(P);
	}
	ad[leaf_pos].sz = (u64)temp_addr - (u64)data_buf_cur;//ad[leaf_pos].addr;
	//LOG(2)<<ad[leaf_pos].sz<<" "<<leaf->data_capacity_;
	ad[leaf_pos].addr = (u64)data_buf_cur; //修改原始结构
	ad[leaf_pos].data_capacity = leaf->data_capacity_;
	data_buf_cur = temp_addr;

	temp_addr = struct_buf_start;
	temp_addr += sizeof(address) * leaf_pos; //修改RDMA区域内的结构
	memcpy(temp_addr, &ad[leaf_pos], sizeof(address));
	
	if(!is_split)
	{
	if(leaf_pos == 18128)
	{
		RDMA_LOG(4)<<(u64)leaf_model_buf_start;
	}
	temp_addr = leaf_model_buf_start;//修改模型区域中的对应参数
         
	temp_addr += sizeof(int);
	memcpy(temp_addr, &leaf->model_.a_, sizeof(double));
	temp_addr += sizeof(double);	
	memcpy(temp_addr, &leaf->model_.b_, sizeof(double));
	temp_addr += sizeof(double) + 2 * sizeof(int);
	memcpy(temp_addr,&leaf->min_error,sizeof(int));
	temp_addr += sizeof(int);
	memcpy(temp_addr,&leaf->max_error,sizeof(int));
	}
}
  std::vector<int> change_pos;
  void change_at_point(int leaf_pos,data_node_type *leaf)
  { 
    	char *temp_addr = reinterpret_cast<char *>(ad[leaf_pos].addr);
    	temp_addr += sizeof(int);
      memcpy(temp_addr, &leaf->version, sizeof(int));
      ...
      {
        temp_addr1 = temp_addr + change_pos[i] * sizeof(T);
        memcpy(temp_addr1,&leaf->key_slots_[change_pos[i]],sizeof(T));
        temp_addr1 = temp_addr + leaf->data_capacity_ * sizeof(T) + change_pos[i] * sizeof(P);
        memcpy(temp_addr1,&leaf->payload_slots_[change_pos[i]],sizeof(P));        
      }
      LOG(2)<<"change_pos size is "<<change_pos.size();
  }
  bool insert_buf(int no, int parent_no, char* RDMA_Key_Addr, char* RDMA_Payload_Addr,char* leaf_model_buf_start,int struct_no){
	
	data_node_type* leaf =static_cast<data_node_type*>(node_index[no]);	
	/*if(no == 23)
	{
		LOG(2)<<"the min err is "<<leaf->min_error<<" "<<"max err is "<<leaf->max_error;
		LOG(2)<<"a is "<<leaf->model_.a_<<" b is "<<leaf->model_.b_;
	}*/
  change_pos.clear();
	bool expand_root_flag = false;
	T tmp_key_buf[buf_size];
  P tmp_payload_buf[buf_size];
  split_node.clear();   
	std::vector<data_node_type*> resize_node;
	memcpy(tmp_key_buf,leaf->key_buf,sizeof(T) * buf_size);
	memcpy(tmp_payload_buf, leaf->payload_buf,sizeof(P) * buf_size);
	memset(leaf->key_buf,-1,sizeof(leaf->key_buf));
  memset(leaf->payload_buf, -1, sizeof(leaf->payload_buf));
  leaf->buf_ptr = 0;	  
	int split_flag = 0;
	...
	bool is_split = false;
  int resize_early_split = 0;
  r2::Timer tt;
	for(int i = 0; i < buf_size; i++)
	{      
    
		T key = tmp_key_buf[i];
		P payload = tmp_payload_buf[i];
    //LOG(2)<<ad[struct_no].addr<<" "<<struct_no<<" "<<key;
		if(split_flag == 1)//索引结构发生过分裂，则需从根节点重新遍历一遍
		{
			std::vector<TraversalNode> traversal_path;
			get_leaf(key, &traversal_path);
			model_node_type *parent = traversal_path.back().node;
		...
		if (key > istats_.key_domain_max_) {
			...
			if (should_expand_right()) {
				expand_root(key, false);
				set_no_with_address(root_node_);	
				expand_root_flag = true;
				expand_flag = true;
		  }
    }
    ...
    if(fail == 0)//成功插入，直接修改内存中的索引和RDMA区域。	
		{  
			if(leaf->num_resizes_ > tmp)
			{	 
					if (is_split == true)//增量索引和学习索引整合导致了分裂
					{
						leaf->version++;
						resize_flag = true;
						bool is_exist = false;
            for(int i = 0; i < resize_node.size();i++)//如果该节点已经扩展过，则处理一次即可
						{
               if(leaf->no == resize_node[i]->no)
						   {
						      is_exist = true;
                  break;
						   }
		        ...
		        if(!is_exist)//将需扩展的节点加入待扩展的集合中				
					     resize_node.push_back(leaf);
						RDMA_LOG(4)<<"node resize after split "<<leaf->no;
					}
					if (!is_split)
					{
						leaf->version++;
						resize_flag = true;//标记已经发生过扩展的情况
			       RDMA_LOG(4)<<"node resize  "<<leaf->no;
					}
					// char *temp_addr = reinterpret_cast<char *>(ad[struct_no].addr); //当客户端进行RDMA读时，就需要重新拉模型。
					// //LOG(2)<<(u64)temp_addr<<" "<<struct_no;
          // temp_addr+=sizeof(int);
          // memcpy(temp_addr,&leaf->version,sizeof(int));
				  // //LOG(2)<<"resize no is "<<leaf->no<<" "<<leaf->model_.a_<<" "<<leaf->model_.b_;
				continue;
			}
      else
      {
        change_pos.push_back(insert_pos);
      }
                 /*       if(leaf->no == 1450)
			{LOG(2)<<"i is "<<i<<"key is "<<key;
				if(i%10 == 0)
					std::cout<<std::endl;
			}/* 放到最后面统一更新最大最小误差
                        if(insert_pos - predict_pos > leaf->max_error)//成功更新需要更新最大最小误差
			{
			   char *temp_addr = reinterpret_cast<char *>(model_address[leaf->no]);
			   leaf->max_error = insert_pos - predict_pos;  
			   memcpy(temp_addr + 2 * sizeof(double) + 4 * sizeof(int),&leaf->max_error,sizeof(int));
			   
			}
			if(insert_pos - predict_pos < leaf->min_error)
			{
			   leaf->min_error = insert_pos - predict_pos;
			   char *temp_addr = reinterpret_cast<char *>(model_address[leaf->no]);
                           memcpy(temp_addr + 2 * sizeof(double) + 3 * sizeof(int),&leaf->min_error,sizeof(int));
			}*/
		 ...
		 if (fail) {                            
	   		 model_node_type *parent; //=static_cast<model_node_type*>(node_index[parent_no]);				
			 if(!split_flag)
			 {
				 /*model_node_type **/parent = static_cast<model_node_type *>(node_index[parent_no]);
			 }
			 ...
			 {	
	          ...
	          while (fail) 
		        {
              LOG(2)<<"The fail is "<<fail;
			       auto start_time = std::chrono::high_resolution_clock::now();
			       stats_.num_expand_and_scales += leaf->num_resizes_;			       
			       if (parent == superroot_) {//只有一个根节点的情况			                    
				       update_superroot_key_domain();			                                                        
			       }			                                             
			       int bucketID = parent->model_.predict(key);//利用父节点预测该键的位置			        
			       bucketID = std::min<int>(std::max<int>(bucketID,0),
					       parent->num_children_ - 1);			                                                  
			       std::vector<fanout_tree::FTNode> used_fanout_tree_nodes;
			       int fanout_tree_depth = 1;	       
				  ...
				  fanout_tree_depth = fanout_tree::find_best_fanout_existing_node<T, P>(
			       ...
			       if (fanout_tree_depth == 0) {
					...
					resize_flag = true;
				//	if(leaf->no == 724)
					LOG(2)<<"resize no is "<<leaf->no<<" "<<key;
					//ASSERT(false);
					if(no == 15388)
					{
						RDMA_LOG(4)<<"The capa of 14108 is "<<leaf->data_capacity_;
					}
					char *temp_addr = reinterpret_cast<char *>(ad[leaf->struct_no].addr); //当客户端进行RDMA读时，就需要重新拉模型。
				  temp_addr+=sizeof(int);
					memcpy(temp_addr,&leaf->version,sizeof(int));
					if (is_split == true)
					{
						 /*resize_after_split tmp;
						  tmp.no = leaf->no;
						for (int i = 0; i < split_node.size(); i++)
						{
							if (split_node[i].leaf->no == leaf->no)
							{
								//tmp.struct_no = split_node[i].leaf_pos;
								tmp.struct_no = split_node[i].leaf->struct_no;
								break;
							}
				...
				/*	if (!is_split)
					 ...
					 if (should_split_downwards) {
						// ASSERT(false);
						LOG(2)<<"warnnnnnnnnnnnnnnnnnnnnnnnn split downward";
					//	LOG(4)<<"i is "<<i<<" key is "<<key;
						/*ad.erase(ad.begin() + leaf->struct_no);
                                                char *temp_addr = reinterpret_cast<char *>(ad[leaf->struct_no].addr);
						  
						temp_addr += sizeof(int);                                       //更改RDMA区域内的版本号
						int temp_version = 999;
						memcpy(temp_addr, &temp_version, sizeof(int));
                                                LOG(4)<<"leaf struct no " <<leaf->struct_no;
						LOG(4)<<"struct no is "<<struct_no;*/
						...
						if (is_split)					
						{
							for (int i = 0; i < split_node.size(); i++)
							{
								if (split_node[i].leaf->no == leaf->no)
								{
									struct_no = split_node[i].leaf->struct_no;
									split_node.erase(split_node.begin() + i);
								}
							}
						}
						...
						parent = split_downwards(parent, bucketID, fanout_tree_depth,
						 ...
						 /*if (is_split)
		   				 {
							 for (int i = 0; i < split_node.size(); i++)
							 {
								 if (split_node[i].leaf->no == parent->no)
								 {
									 struct_no = split_node[i].leaf->struct_no;
									 split_node.erase(split_node.begin() + i);
								 }
						 ...
						 //当原来的数据节点转化为模型节点，则将其版本号置为999，并修改对应RDMA区域内的对应值
	           char *temp_addr = reinterpret_cast<char *>(ad[struct_no].addr); //当客户端进行RDMA读时，就需要重新拉模型。
						 temp_addr += sizeof(int);
						 memcpy(temp_addr, &parent->version, sizeof(int));
						// LOG(2)<<ad.at(struct_no).no;
						 ad.erase(ad.begin() + struct_no); //重新设置编号，可不可以用另一个线程？					
						 temp_addr = model_buf_start; //重置RDMA模型区域
						 auto node = root_node_;
						// LOG(2)<<"split downward!!!";
						 set_no_with_address(root_node_);
					 } else {
			        		 RDMA_LOG(4)<<"split leaf no is " <<leaf->no;
						 //LOG(2)<<"key is "<<key;
						 if(is_split)
						 {
							 for(int i = 0; i < split_node.size();i++)		
						 ...
						 char *temp_addr = reinterpret_cast<char *>(ad[leaf->struct_no].addr); //当客户端进行RDMA读时，就需要重新拉模型。
						 int uu = 0;
						 memcpy(&uu,temp_addr,sizeof(int));
						 LOG(2)<<uu;
	    					 temp_addr += sizeof(int);                                       //更改RDMA区域内的版本号
						 LOG(2)<<ad[leaf->struct_no].addr<<" "<<ad[struct_no].no;
						 int temp_version = 999;
						 memcpy(temp_addr, &temp_version, sizeof(int));
						 split_sideways(parent, bucketID, fanout_tree_depth,
								 used_fanout_tree_nodes, reuse_model);
						  ...
						  temp_addr += sizeof(int);                                       //更改RDMA区域内的版本号
    			         		  int temp_version = 999;	
						  memcpy(temp_addr, &temp_version, sizeof(int));*/
						  //LOG(2)<<"ooooooooooooooooooooooo "<<ad.at(struct_no).no;
						  ad.erase(ad.begin() + struct_no); //删除结构中的分裂前的元素
						  temp_addr = model_buf_start; //重置RDMA模型区域
						  auto node = root_node_;
                                                  int tmp_size;
						  tmp_size = split_node.size();						  
						  ...
						  /*for(int i = 0; i < tmp_size; i++)
							  ...
							  {
								  if(split_node[i].leaf->no == ad[j].no)
								  {
									  split_node[i].leaf_pos = j;
									  break;
								  }
							  }
						  }*/
	...
	stats_.num_inserts++;
	stats_.num_keys++;
	/*RDMA_LOG(4)<<"split node size is "<<split_node.size();
	for(int i = 0; i < split_node.size(); i++)    
	{
		if(split_node.at(i).leaf->no==17162)
		{
			RDMA_LOG(4)<<ad[split_node.at(i).leaf_pos].addr ;
			char* tp_addr = reinterpret_cast<char*>(ad[split_node.at(i).leaf_pos].addr);
			int tp_key;
			memcpy(&tp_key,tp_addr,sizeof(int));
			RDMA_LOG(4)<<tp_key;
		}													
	}*/

}  
//LOG(2)<<"整合150个key的时间是 "<<tt.passed_msec()<<" msec";
tt.reset();
if(resize_flag == true&&!is_split)
{
	char* temp_addr =reinterpret_cast<char*>(model_address[leaf->no]);
/*	if(no == 15388)
	{
		RDMA_LOG(4)<<"struct_no is "<<struct_no;
		RDMA_LOG(4)<<"ad.size is "<<ad.size();
		RDMA_LOG(4)<<"The ad.addr is "<<ad[struct_no].addr;
		RDMA_LOG(4)<<"The model address "<<model_address.size();
		RDMA_LOG(4)<<"the leaf model "<<model_address[leaf->no-1];
	}
*/
	//char* temp_addr = model_address[leaf->no];
	resize_RDMA_Region(leaf->struct_no,leaf,temp_addr,RDMA_Key_Addr,RDMA_Payload_Addr,is_split);
  //RDMA_LOG(4)<<"no "<<leaf->no<<" resized"<<" "<<ad[leaf->struct_no].addr;
}
//LOG(2)<<"the resize RDMA region time is "<<tt.passed_msec()<<" msec";

tt.reset();
if(is_split||expand_root_flag)
{
	auto node = root_node_;
	char* temp_addr = model_buf_start;
	LOG(2)<<"the data buf cur "<<(u64)data_buf_cur;
  RDMA_LOG(4)<<"temp_addr is "<<(u64)temp_addr;
	std::queue<AlexNode<T, P> *> myqueue;
	myqueue.push(node);
	int repeats = 0;
	int nu_node = 0;
  ...
  while (1)
	...
	/*for(int i = 0; i < split_node.size(); i++)
	{
	        ...
	        if(split_node.at(i).leaf->no==15118)
		{
			//RDMA_LOG(4)<<ad[split_node.at(i).leaf_pos].addr ;
		        char* tp_addr = reinterpret_cast<char*>(ad[split_node.at(i).leaf->struct_no].addr);
                        int tp_key;
			memcpy(&tp_key,tp_addr,sizeof(int));
			RDMA_LOG(4)<<tp_key;
		}
		//RDMA_LOG(4)<<"no "<<split_node.at(i).leaf->no<<" splited";
	}*/
  LOG(2)<<"The resize_node.size "<<resize_node.size();
	for(int i = 0; i <resize_node.size();i++)
	{
		//auto cur = static_cast<data_node_type*>(node_index[resize_no[i].no]);
		auto cur = resize_node[i];
		//RDMA_LOG(4)<<"resize_no is "<<cur->no;
	/*	if(resize_node.at(0)->no == 14108)
		{
			RDMA_LOG(4)<<"split node size is "<<split_node.size();
			for(int i = 0; i < split_node.size(); i++)
			{
				RDMA_LOG(4)<<split_node[i].leaf->no;
			}
		}*/
		for(int j = 0; i < split_node.size(); j++)
		{
			if(split_node.at(j).leaf->no == resize_node[i]->no)
			{
				...
				break;
                   ...
                   /*      if( cur->no == 14108)                        
		         {
				RDMA_LOG(4)<<"node index "<<static_cast<data_node_type*>(node_index[cur->no])->data_capacity_;
				 RDMA_LOG(4)<<cur->data_capacity_;
...
/*        if(resize_node.at(0)->no == 14108)
  ...
  */
  for(int i = 0; i < split_node.size(); i++)
	{
	  LOG(2)<<"split node no is "<<split_node[i].leaf->no;
		change_RDMA_Region(split_node.at(i).leaf->struct_no,split_node.at(i).leaf);
		LOG(2)<<split_node.at(i).leaf->struct_no;	            
		/*if(split_node.at(i).leaf->no==15118)						
		{			
			char* ptr = reinterpret_cast<char*>(ad[split_node.at(i).leaf_pos].addr) ;
			RDMA_LOG(4)<<ad[split_node.at(i).leaf_pos].addr ;
			int tp_ca;
			int tp_key;
			memcpy(&tp_ca,ptr,sizeof(int));
	                RDMA_LOG(4)<<"The capacity is "<<tp_ca;
			ptr+=2*sizeof(int);
		   	for(int j = 0; j < tp_ca;j++)
			{
			  memcpy(&tp_key,ptr,sizeof(int));
			  RDMA_LOG(4)<<"tp_key is "<<tp_key;
			  ptr+=sizeof(int);
			}

		}*/
	}
  //              for(int i = 850;i <ad.size() ;i++)
  //            {
  //              LOG(2)<<"The no is "<<ad[i].no;
  //              if(ad[i].no == 876)
  //              {
  //                char* ry = (char*)ad[i].addr;
  //                int oo;
  ...
  //             LOG(2)<<"version is "<<oo;
  //              }
  //              LOG(2)<<"The addr is "<<ad[i].addr;
  //            }
	// std::cout<<std::endl;
}
 //   LOG(2)<<"deal with split cost "<<tt.passed_msec()<<" msec";
    tt.reset();
/*}
else
{
		...
		char* temp_addr = model_buf_start;
		RDMA_LOG(4)<<"temp_addr is "<<(u64)temp_addr;
		std::queue<AlexNode<int, int> *> myqueue;
		myqueue.push(node);
		int repeats = 0;
		int nu_node = 0;
		int r = 0;
		while (1)
		{
			if (!node->is_leaf_)
			{
				...
				memcpy(temp_addr, &cur->num_children_, sizeof(int));
				temp_addr += sizeof(int);
			//	RDMA_LOG(4)<<"temp_addr size is "<<temp_addr - model_buf_start;
			//	RDMA_LOG(4)<<"the num_childre is "<<cur->num_children_;
				for (int i = 0; i < cur->num_children_; i++)
				{
					myqueue.push(cur->children_[i]);
					repeats = 1 << cur->children_[i]->duplication_factor_;
			...
			//	RDMA_LOG(4)<<"The no is "<<cur->no;
			//	RDMA_LOG(4)<<"The model nodel num_children is "<<r;
			//	RDMA_LOG(4)<<"temp_addr size is "<<temp_addr - model_buf_start;
				nu_node++;
			}
			else
			{
				auto cur = static_cast<data_node_type*>(node);
				memcpy(temp_addr, &cur->no, sizeof(int));
				if(cur->no == resize_no[0])
				{
					leaf_model_buf_start = temp_addr;
				}
				temp_addr += sizeof(int);
				memcpy(temp_addr, &cur->model_.a_, sizeof(double));
				temp_addr += sizeof(double);
			...
			node = myqueue.front();
	               ...
	               white_size = struct_buf_start - temp_addr;
		       address ce;
		       memcpy(&ce,temp_addr+white_size,sizeof(address));
		       RDMA_LOG(4)<<"ce.no is "<<ce.no;
		       memcpy(&ce,struct_buf_start,sizeof(address));
		       RDMA_LOG(4)<<"model_buf_start.no is "<<ce.no;
		
	}
*/
     if(!split_flag&&!resize_flag)
     {
        // char* ry = (char*)ad[864].addr;
        //          int oo;
        //          memcpy(&oo,ry,sizeof(int));
        //          LOG(2)<<"更新前no is "<<oo;
              //memcpy(&oo,ry +sizeof(int),sizeof(int));
              //LOG(2)<<"version is "<<oo;
	    // change_RDMA_Region(leaf->struct_no,leaf);
       //change_at_point(leaf->struct_no,leaf);
        // memcpy(&oo,ry,sizeof(int));
        //          LOG(2)<<"更新后no is "<<struct_no<<" "<<oo<<" "<<struct_no;
     } 
    // LOG(2)<<"cost time of only change is "<<tt.passed_msec();
     tt.reset();
     if(split_flag != 1)
	{
		memset(leaf->key_buf, -1, sizeof(leaf -> key_buf));
		...
		leaf->buf_ptr = 0;
           ...
           if(model_buf_start - data_buf_cur < 10000)
	         {		LOG(2)<<(u64)model_buf_start;
LOG(2)<<(u64)data_buf_cur;			 
LOG(2)<<model_buf_start - data_buf_cur;
ASSERT(false);
			 reset_data_region();
		 }
       // LOG(2)<<"other time cost is "<<tt.passed_msec();
        /*for(int i = 0 ; i < update_error.size(); i++)//整合完毕后，更新每个节点的最大最小误差
        {
