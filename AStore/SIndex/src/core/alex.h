...
// node's parent.
 ...
 private:
  ...
  // dense keys.
  //void bulk_load_from_RDMA(const V  values[],)
 ...
 private:
  ...
  //将数据节点一分为二，并沿遍历路径向上传播拆分。
  //在两个新创建的数据节点中，返回key所在的节点。
 //通过new_parent返回新数据节点的父模型节点。
 ...
 public:
  // Erases the left-most key with the given key value
     //从叶子节点找到删除的key，将其删除后后面的key依次前进一位
  int erase_one(const T& key) {
    data_node_type* leaf = get_leaf(key);
    int num_erased = leaf->erase_one(key);//返回1
    stats_.num_keys -= num_erased;
    if (leaf->num_keys_ == 0) { // 当前叶子无键，则合并
      merge(leaf, key);
    }
    if (key > istats_.key_domain_max_) { //更新参数
 ...
 private:
  ...
  void merge(data_node_type* leaf, T key) {
    ...
    std::vector<TraversalNode> traversal_path;
    auto leaf_dup = get_leaf(key, &traversal_path);//获取当前key所在的叶节点以及遍历路径
