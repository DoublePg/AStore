...
public:
  ...
  */
  ...
  /*!
    ...
    打开一个nic处理器，如果成功valid()返回真，否则返回错误信息
   */
  ...
  // implementations
private:
  struct ibv_context *open_device(const DevIdx &idx) {//打开对应ID和端口号的设备

    ibv_context *ret = nullptr;

    int num_devices;
    struct ibv_device **dev_list = ibv_get_device_list(&num_devices);//num_devices存储nic设备的数量，返回RNIC设备的一个二位数组
  ...
  struct ibv_pd *alloc_pd() {
    if (ctx == nullptr)
      return nullptr;
    auto ret = ibv_alloc_pd(ctx);//ibv_alloc_pd创建一个保护域（PD）。 PD限制可以通过哪些队列对（QP）提供一定程度的保护以防止未经授权的访问来访问哪些存储器区域。
