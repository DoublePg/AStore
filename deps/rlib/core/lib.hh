...
RCtrl(在rctrl.hh中定义)为在服务器上调用这些函数提供端点支持。用户也可以注册他们指定的回调，详情请参见rctrl.hh。

示例用法(获取由ID标识的远程注册MR)。
\注意：我们首先要确保服务器必须**启动**，并且处理程序必须**注册**。否则，所有呼叫都将超时
  `
...
public:
  ...
  /*!
    *C*reate and *C*onnect an RC QP at remote end.
    ...
    created QP.
    *C*Reate和*C*在远程端连接RC QP。
    该函数首先在本地创建RC QP，
    然后在远程主机上创建RC QP以连接本地创建的QP。
    \param:
  ...
  Result<cc_rc_ret_t> cc_rc(const std::string &name,
    ...
    {
      proto::RCReq req = {};
      memcpy(req.name, name.data(), name.size());//将名字复制到请求中
      ...
      auto res =
          rpc.call(proto::CreateRC, ::rdmaio::Marshal::dump<proto::RCReq>(req));//服务器端调用创建RC的命令
      ...
      auto res_reply = rpc.receive_reply(timeout_usec);//接受回应
      ...
      if (res_reply == IOCode::Ok) {
        try {
          ...
          switch (qp_reply.status) {
          case proto::CallbackStatus::Ok: {
            auto ret = rc->connect(qp_reply.attr);//连接远端qp
            if (ret != IOCode::Ok) {
              err_str = ret.desc;
              goto ErrCase;
            }
            auto key = qp_reply.key;//获取远端密钥
  ...
  /*!
    ...
    在此连接管理器的远程机器上获取标识为“id”的远程MR，并将结果存储在“attr”中。
   */
  ...
  Result<qp_attr_ret_t> fetch_qp_attr(const std::string &name,
    ...
    auto err_str = std::string("unknown error");
    {
      // 1. first, sanity check the arg检查参数
      if (unlikely(name.size() > ::rdmaio::qp::kMaxQPNameLen)) {
        err_str = err_name_to_long;
        goto ErrCase;
      }
